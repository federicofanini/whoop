...
import { hrZones, series } from "@/lib/theme";
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={chartMargin} barCategoryGap="18%">
            ...
            <XAxis
              ...
              minTickGap={28}
            />
            <YAxis {...axisProps(tokens)} width={30} domain={[0, 21]} ticks={[0, 7, 14, 21]} />
    ...
    <div style={{ height }}>
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} layout="vertical" margin={{ top: 4, right: 16, bottom: 4, left: 4 }}>
          <CartesianGrid {...gridProps(tokens)} vertical horizontal={false} />
          <XAxis
            type="number"
            {...axisProps(tokens)}
          ...
          />
          <YAxis type="category" dataKey="label" {...axisProps(tokens)} width={56} />
            ...
            content={({ active, payload }) => {
              ...
              return (
                <TooltipShell
                  title={point.label}
                  rows={[
                    { label: t("sleepPage.time"), value: t.duration(point.seconds * 1000), color: point.color },
                    {
                      label: "Share",
